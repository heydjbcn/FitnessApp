...
struct SettingsDayView: View {
    ...
    var isFormValid: Bool {
        !machineName.isEmpty && !repetitions.isEmpty && !weight.isEmpty && !sets.isEmpty && !selectedDays.isEmpty
    }

    // Helper function para evitar problemas de @MainActor
                ...
                TextEditor(text: $infoText)
                    ...
                    .overlay(
                        RoundedRectangle(cornerRadius: 12)
                            .stroke(getPrimaryColor().opacity(0.3), lineWidth: 1)
                    )
            }
            ...
            ForEach(WorkoutDay.allCases) { day in
                Button(action: {
                    ...
                    Text(day.rawValue.prefix(3).uppercased())
                        ...
                        .background(selectedDays.contains(day) ? getPrimaryColor() : AppColors.cardBackground(isDark: themeManager.isDarkMode))
                        .cornerRadius(10)
                        .shadow(
                            color: selectedDays.contains(day) ? getPrimaryColor().opacity(0.6) : .clear,
                            radius: 6,
                            x: 0,
                            y: 3
                        )
    ...
    private func toggleDaySelection(_ day: WorkoutDay) {
    ...
    private func addExerciseToSelectedDays() {
        guard let reps = Int(repetitions),
              let weightValue = Double(weight),
              let setsValue = Int(sets) else { return }
